...
(defmethod proxy ((inner iter) (cont function))
  ...
  (let ((res))
    ...
    (if (null (iter-next inner))
        ...
        (let* ((skip (funcall cont))
              ; (skip-next)
               )
          (print skip)
          ;(setf skip-next (funcall (pass-cont skip)))
          ;(setf (pass-results skip-next) (pass-results skip))
          skip)
        (make-pass :cont (lambda () (print "here") (proxy inner cont)) :results res)
        ;; (make-pass :cont (lambda (&optional (x nil supplied))
        ;;                    (if supplied
        ;;                        (proxy inner (lambda () (funcall cont x)))
        ;;                        (proxy inner cont)))
        ;;            :results res)
        ))
  )
